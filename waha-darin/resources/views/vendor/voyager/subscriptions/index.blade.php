...
@section('content')
<div class="page-content container-fluid" id="subscriptions-app">
    <div class="sub-header">
        <div class="sub-title">
            <h1 class="page-title mb-0">{{ __('Subscriptions') }}</h1>
            <p class="text-muted mb-0">{{ __('Manage pending, activated, and deactivated subscriptions.') }}</p>
        </div>
...
@section('css')
<style>
    #subscriptions-app { padding: 18px 24px; }
    #subscriptions-app .sub-header { display:flex; align-items:flex-start; justify-content:space-between; gap:12px; margin-bottom:16px; }
    #subscriptions-app .sub-shell { background:#fff; border:1px solid rgba(0,0,0,0.08); border-radius:14px; overflow:hidden; }
    #subscriptions-app .sub-loading { padding:28px; text-align:center; color:#667085; }

    #subscriptions-app .sub-tabs { display:flex; gap:10px; padding:12px 14px; background:#f7f7f7; border-bottom:1px solid rgba(0,0,0,0.06); }
    #subscriptions-app .sub-tab { border:1px solid rgba(0,0,0,0.10); background:#fff; color:#344054; border-radius:999px; padding:10px 14px; font-weight:700; display:inline-flex; align-items:center; gap:10px; transition: background .15s ease, border-color .15s ease; }
    #subscriptions-app .sub-tab:hover { background: rgba(0,0,0,0.02); }
    #subscriptions-app .sub-tab.is-active { border-color: rgba(0,0,0,0.20); box-shadow: 0 1px 0 rgba(0,0,0,0.06); }
    #subscriptions-app .sub-count { display:inline-flex; align-items:center; justify-content:center; min-width:22px; height:22px; padding:0 6px; border-radius:999px; background: rgba(0,0,0,0.07); font-size:12px; font-weight:800; }
    #subscriptions-app .sub-tab[data-tab="pending"] { background:#fef3c7; color:#92400e; border-color:#fde68a; }
    #subscriptions-app .sub-tab[data-tab="active"] { background:#d1fae5; color:#065f46; border-color:#a7f3d0; }
    #subscriptions-app .sub-tab[data-tab="deactivated"] { background:#fee2e2; color:#991b1b; border-color:#fecaca; }
    #subscriptions-app .sub-tab[data-tab="pending"] .sub-count { background:rgba(146,64,14,0.14); }
    #subscriptions-app .sub-tab[data-tab="active"] .sub-count { background:rgba(6,95,70,0.14); }
    #subscriptions-app .sub-tab[data-tab="deactivated"] .sub-count { background:rgba(153,27,27,0.14); }

    #subscriptions-app .sub-panels { padding:14px; }
    #subscriptions-app .sub-panel { display:none; margin:0; padding:0; }
    #subscriptions-app .sub-panel.is-active { display:block; }

    #subscriptions-app details.sub-card { background:#fff; border:1px solid rgba(0,0,0,0.08); border-radius:14px; overflow:hidden; box-shadow: 0 1px 2px rgba(16,24,40,0.06); margin:0 0 14px; }
    #subscriptions-app details.sub-card summary::-webkit-details-marker { display:none; }
    #subscriptions-app .sub-card__summary { cursor:pointer; user-select:none; padding:14px 16px; display:flex; justify-content:space-between; align-items:center; gap:12px; border-bottom:1px solid rgba(0,0,0,0.06); }
    #subscriptions-app .sub-card__left { display:flex; align-items:center; gap:12px; min-width:0; }
    #subscriptions-app .sub-card__chev { width:34px; height:34px; border-radius:10px; border:1px solid rgba(0,0,0,0.10); display:flex; align-items:center; justify-content:center; color:#475467; flex:0 0 auto; background:#fff; }
    #subscriptions-app details.sub-card[open] .sub-card__chev i { transform: rotate(180deg); display:inline-block; transition: transform .15s ease; }
    #subscriptions-app .sub-card__meta { min-width:0; }
    #subscriptions-app .sub-card__title { font-weight:800; color:#101828; line-height:1.2; }
    #subscriptions-app .sub-card__sub { font-size:12px; line-height:1.2; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; max-width:60vw; }
    #subscriptions-app .sub-card__right { display:flex; align-items:center; gap:10px; flex:0 0 auto; text-align:right; }
    ...
    #subscriptions-app .sub-block__title { font-weight:800; font-size:12px; color:#475467; text-transform:uppercase; letter-spacing:.04em; margin-bottom:8px; }
    #subscriptions-app .sub-divider { height:1px; background: rgba(0,0,0,0.08); margin:14px 0; }
    #subscriptions-app .sub-buttons { display:flex; gap:10px; flex-wrap:wrap; margin-top:6px; }
    #subscriptions-app .sub-buttons .btn { border-radius:12px; font-weight:800; }

    @media (max-width: 640px) {
        #subscriptions-app { padding:16px; }
        #subscriptions-app .sub-card__right { flex-direction:column; align-items:flex-end; }
    }
</style>
@endsection
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const app = document.getElementById('subscriptions-app');
        if (!app) return;
        ...
        const endpoints = {
            index: '{{ route('voyager.subscriptions.data') }}',
            activate: function (id) { return '{{ url('/admin/subscriptions') }}' + '/' + id + '/activate'; },
            deactivate: function (id) { return '{{ url('/admin/subscriptions') }}' + '/' + id + '/deactivate'; },
        };
        ...
        function renderActive() {
            ...
            items.forEach(function (sub) {
                ...
                populateSubscription(fragment, sub, listType);
        ...
        function populateSubscription(fragment, sub, listType) {
            const user = sub.user || {};
            const plan = sub.plan || {};
            ...
            // Deactivate is allowed from pending/active pages (and even from any tab).
            // Activate is allowed from pending (and optionally from deactivated if needed later).
            activateBtn.style.display = (status === 'active') ? 'none' : '';
            deactivateBtn.style.display = (status === 'deactivated') ? 'none' : '';
